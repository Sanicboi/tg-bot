...
export class Bot {
    ...
    public static async reply(bot: TelegramBot, repo: Repository<Answer>, skipped: Iskipped[], user: User, openai: OpenAIApi, q: CallbackQuery, valuation: number) {
        try {
	        ...
	        Артикул: ${current.productDetails.nmId}
    ...
    public static async confirm(user: User, id: number, admins: Admin[], bot: TelegramBot, q: CallbackQuery, repo: Repository<Answer>) {
        try {
            const result = await AnswerWrapper.confirm(id, user, repo);
            ...
            admins.forEach(async admin => {
                await bot.sendMessage(Number(admin.chatId), `#ответсогласованотправлен ${user.name} @${q.from.username || q.from.first_name} Промокод: ${user.special ? 'special23' : 'ранняяпташка'}`);
            });
        } catch (error) {
            console.log(error);
            await bot.sendMessage(q.from.id, 'Произошла ошибка', {
    ...
    public static async edit(msg: Message, admins: Admin[], bot: TelegramBot, user: User, text: string,  repo: Repository<Answer>) {
        try {
            ...
            admins.forEach(async admin => {
                await bot.sendMessage(Number(admin.chatId), `#ответизменен ${user.name} @${msg.from?.username || msg.from?.first_name} Промокод: ${user.special ? 'special23' : 'ранняяпташка'}`);
            });
        } catch (error) {
            console.log(error);
    ...
    public static async newHome(msg: TelegramBot.Message, bot: TelegramBot, user: User) {
        try {
            ...
            console.log(error);
            await bot.sendMessage(msg.chat.id, 'Не введен стандартный токен ВБ.\nПожалуйста, введите токен здесь: /me', {
                reply_markup: {
                    inline_keyboard: [
                        [
                            {
